        ...
        if (data.join) {
            ...
            var roomdiv = $(
                "<div class='room' id='room-" + data.join + "'>" +
                 "<div class='room-name'>"+ data.title +
                "</div>"+
                "<div class='messages'></div>" +
               "<div class='message-input row'>"+
			"<input type='text' class='col-md-11 input-msg'  autofocus placeholder='Write your message...'/>"+
			"<button class='submit btn btn-send col-md-1'><i class='fa fa-paper-plane' aria-hidden='true'></i></button>"+
			"</div> "+
                "</div>"
            );
            ...
            roomdiv.find("input").on("keydown", function(e){
                if(e.which == 13){
                socket.send(JSON.stringify({
                    "command": "send",
                    "room": data.join,
                    ...
                    if(username == data.username){
                       ...
                       ok_msg +=
                        "<span class='username-right'>" + data.username + "</span>" +
                           "<p class='dark'>" + data.message  + "<br><br><span class='time-right small'>"+currentTime + "</span></p>"+
                        "</div></div>";
                    }
                    else {
                        ok_msg += "<span class='username'>" + data.username + "</span>" +
                           "<p class='light'>" + data.message  + "<br><br><span class='time-left small'>"+currentTime + "</span></p>"+
                        "</div></div>";
                    }
                    break;
                case 1:
